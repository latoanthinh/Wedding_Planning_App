...
const GiftDetail = (props) => {
  ...
  const { GitflId, item } = route?.params || {};
  ...
  useEffect(() => {
    if ((ChitietGiftStatus === 'succeeded' && ChitietGiftData) || item) {
  ...
  }, [ChitietGiftStatus, ChitietGiftData, item]);

  const handleToggleFavorite = () => {
    if (!GitflId || !user || !user._id) {
    ...
    if (isFavorite) {
      dispatch(removeFavoriteItem({ userId: user._id, type: 'present', itemId: GitflId }))
        .unwrap()
        .then(() => {
          setIsFavorite(false);
      ...
      dispatch(addFavoriteItem({ userId: user._id, type: 'present', itemId: GitflId }))
        .unwrap()
        .then(() => {
          setIsFavorite(true);
  ...
  if (!item && ChitietGiftStatus === 'loading') {
    return (
      <SafeAreaView style={styles.container}>
  ...
  if (!item && ChitietGiftStatus === 'failed') {
    return (
      <SafeAreaView style={styles.container}>
  ...
  const displayData = item || ChitietGiftData;
  if (!displayData) {
    return (
      <SafeAreaView style={styles.container}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.imageContainer}>
        <Animated.View style={{ opacity: imageAnim, transform: [{ scale: imageAnim.interpolate({ inputRange: [0, 1], outputRange: [0.9, 1] }) }] }}>
          <Image 
        ...
        <View style={styles.headerButtons}>
          ...
          <TouchableOpacity style={styles.favoriteButton} onPress={handleToggleFavorite} disabled={isFavoriteLoading}>
            {isFavoriteLoading ? (
      ...
      <View style={styles.contentCard}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
          <View style={styles.titleContainer}>
            <Animated.Text style={[styles.giftName, { opacity: nameAnim, transform: [{ translateY: nameAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }]}>
          ...
          <View style={styles.tagsContainer}>
            <View style={styles.tagItem}><Text style={styles.tagText}>{displayData.Cate_presentId?.name || 'Quà tặng'}</Text></View>
            <View style={styles.tagItem}><Text style={styles.tagText}>Quà cưới</Text></View>
            <View style={styles.tagItem}><Text style={styles.tagText}>Lưu niệm</Text></View>
          </View>
          ...
          <View style={styles.featuresSection}>
            ...
            <View style={styles.featureItem}><Icon name="gift" size={20} color="#A67C52" /><Text style={styles.featureText}>Quà tặng cao cấp</Text></View>
            ...
            <View style={styles.featureItem}><Icon name="check-circle" size={20} color="#A67C52" /><Text style={styles.featureText}>Bao gồm dịch vụ gói quà</Text></View>
            <View style={styles.featureItem}><Icon name="clock-outline" size={20} color="#A67C52" /><Text style={styles.featureText}>Thời gian chuẩn bị: 1-2 ngày</Text></View>
          </View>
          <View style={styles.divider} />
