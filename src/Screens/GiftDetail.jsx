...
const GiftDetail = (props) => {
    ...
    const handleToggleFavorite = async () => {
        ...
        try {
            if (isFavorite) {
                await dispatch(removeFavoriteItem({ userId: user._id, type: 'Present', itemId: GitflId })).unwrap();
                setIsFavorite(false);
                ToastAndroid.show('Đã xóa khỏi danh sách yêu thích', ToastAndroid.SHORT);
            } else {
                await dispatch(addFavoriteItem({ userId: user._id, type: 'Present', itemId: GitflId })).unwrap();
                setIsFavorite(true);
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                    <View style={styles.titleContainer}>
                        <Animated.Text
                            style={[
                                ...
                                {
                                    opacity: nameAnim,
                                    transform: [{ translateY: nameAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }],
                                },
                            ]}
                        >
                    ...
                    <View style={styles.tagsContainer}>
                        <View style={styles.tagItem}>
                            <Text style={styles.tagText}>{displayData.Cate_presentId?.name || 'Quà tặng'}</Text>
                        </View>
                        <View style={styles.tagItem}>
                    ...
                    >
                        ...
                        <Text style={styles.descriptionText}>
                            {displayData.Description || displayData.description || 'Không có mô tả'}
                        </Text>
                    </Animated.View>
                    ...
                    <Animated.View
                        style={[
                            ...
                            {
                                opacity: priceAnim,
                                transform: [{ translateY: priceAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }],
                            },
                        ]}
                    >
...
const styles = StyleSheet.create({
    ...
    actionButtons: {
        ...
        paddingVertical: 15,
        borderTopWidth: 1,
        borderTopColor: '#F1E4D8itization',
    },
    contactButton: {
        flex: 1,
        flexDirection: 'row',
